...
import "./chunk-JZY6X4J2.js";
import {
  redirect
} from "./chunk-R66DIGVQ.js";
import {
  require_jsx_runtime
...
var SHOPIFY_REACT_ROUTER_LIBRARY_VERSION = "1.2.0";
...
async function createOrLoadOfflineSession({ api, config, logger }, shop) {
  if (config.distribution === AppDistribution.ShopifyAdmin) {
...
async function refreshToken(params, shop, refreshToken2) {
  var _a;
  const { api } = params;
  try {
...
async function ensureOfflineTokenIsNotExpired(session, params, shop) {
  var _a;
  const { config } = params;
  if (((_a = config.future) == null ? void 0 : _a.expiringOfflineAccessTokens) && session.isExpired(WITHIN_MILLISECONDS_OF_EXPIRY) && config.distribution !== AppDistribution.ShopifyAdmin && session.refreshToken) {
...
var createTokenExchangeStrategy = (params) => {
  ...
  async function exchangeToken({ request, shop, sessionToken, requestedTokenType }) {
    var _a;
    try {
      console.log("config.future.expiringOfflineAccessTokens", config.future.expiringOfflineAccessTokens);
